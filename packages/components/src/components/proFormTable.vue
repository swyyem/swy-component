...
type FormTableProps = PropFormFieldProps & Omit<ProFieldPropsType<'table'>, 'valueType'>
const props = withDefaults(
  defineProps<FormTableProps>(),
  getProFormFieldInitProps(),
) as FormTableProps

const ProFormFieldProps: Ref<PropFormFieldProps> = computed(() => {
  return getProFormFieldProps<'table'>(props)
})

const ProFieldProps: Ref<ProFieldPropsType> = computed(() => {
  return getProFieldProps(props, 'table')
})
