...
export type ActionsPopconfirmProps<T> = ActionInnerPopconfirmProps<T> | ((data?: T) => ActionInnerPopconfirmProps<T>);
export type ActionInnerButtonType = Partial<ButtonProps> & {
    label?: string;
    slot?: DialogSlotType;
};
export type ActionInnerButtonProps<T> = ActionInnerButtonType | ((data?: T) => ActionInnerButtonType);
export type ActionDialogProps<T extends ProComponentObject> = FormDialogType<T>;
export type ActionButtonProps<T extends ProComponentObject = ProComponentObject> = DialogRequestType<T> & {
