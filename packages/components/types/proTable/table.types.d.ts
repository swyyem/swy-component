...
import type { PaginationProps, PaginationEmits, ButtonProps, TableInstance, Column, Columns, FormValidateFailure } from 'element-plus';
...
import type { ProFormProps, ProFormEvents, PropFormFieldProps, TextSpecifiledProps, ProFieldValueType, ProFieldPropsType } from '../index';
...
export type ProTableFieldProps<ValueType extends ProFieldValueType = ProFieldValueType> = Omit<ProFieldPropsType<ValueType>, 'mode' | 'textProps' | 'modelValue' | 'renderFormItem' | 'render'>;
...
export type ProActionButtonProps<T extends ProComponentObject> = false | ProTableButtonProps<T> | ((params: ProCellRendererParams<T>) => false | ProTableButtonProps<T>);
...
export type ProPagerProps = {
    pagination: Partial<PaginationProps> & Partial<PaginationEmits> & {
...
export type ProCellRenderer<T extends ProComponentObject> = (params: ProCellRendererParams<T>) => VNode;
...
export type ProColumn<T extends ProComponentObject> = Omit<TextSpecifiledProps, 'copyText'> & {
    ...
    required?: boolean;
    /** 控件 校验规则 */
    rules?: PropFormFieldProps['rules'] | ((rowIndex: string, rowData: T) => PropFormFieldProps['rules']);
    /** 控件联动规则 */
...
export type ProBaseTableProps<T extends ProComponentObject = ProComponentObject> = Pick<ProTableContentProps<T>, 'highlightCurrentRow'> & {
    /** 外层容器类名 */
    ...
    /** 新建按钮属性 */
    recordCreatorProps?: (Partial<ButtonProps> & {
    ...
    /** proForm 的 props 透传 */
    formProps?: Omit<ProFormProps, 'initialValues' | 'submitter' | 'grid' | 'fieldErrorType'> & ProFormEvents;
...
export type ProTableFormValidateType = (v: boolean, invalidFields: ProTableFormInvalidFieldsType) => void;
