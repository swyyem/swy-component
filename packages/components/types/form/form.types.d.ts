...
import type { ButtonProps, FormProps, FormItemProps, FormValidationResult, FormValidateCallback } from 'element-plus';
import type { ProFieldPropsType, ProSchemaValueEnumType, ProFieldInstance } from '../proField/index.type';
...
export type ProFieldComponentValueType = Extract<keyof ProFieldValueTypeWithFieldComponentProps, unknown> | (string & {});
...
export type EffectType = {
    ...
    /** 异步一对多的场景 */
    batchLogic?: (v: ProFormFieldValue, methods: {
...
export type PropFormFieldProps = Partial<Omit<FormItemProps, 'prop'>> & Pick<ProFieldPropsType, 'mode' | 'fieldProps' | 'valueEnum' | 'request'> & {
...
export type SubmitterProps<T = ProFormValueType> = {
    ...
    render?: ((props: SubmitterProps & T & {
        submit: () => void;
        reset: (e: MouseEvent) => void;
    }, doms: VNode[]) => VNode[] | VNode | false) | false;
};
type SelectOption = {
    label: string;
...
export interface ProFormProps<T extends ProComponentObject = ProComponentObject> extends Partial<Omit<FormProps, 'model'>>, Pick<ProFieldPropsType, 'mode'>, Pick<PropFormFieldProps, 'colon' | 'emptyText'> {
    /** 标签 v-model */
    modelValue?: T;
    /** JSON 配置生成表单项 */
    columns?: FiledForm[];
...
export type FiledForm = PropFormFieldProps & Omit<ProFieldPropsType, 'mode' | 'fieldProps' | 'valueEnum' | 'request' | 'valueType'> & {
